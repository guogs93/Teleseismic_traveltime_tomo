...
REAL(KIND=i10) :: itres,ftres,prs,rd1,rd2,rd3,rdum
REAL(KIND=i10), DIMENSION (:), ALLOCATABLE :: stt,srcm
REAL(KIND=i10), DIMENSION (:), ALLOCATABLE :: mtim,rtim,otim
CHARACTER (LEN=40) :: mtimes,rtimes,otimes,sources,receivers,oifile,offile
...
! oifile = output file for plotting initial residual
! offile = output file for plotting final residual
...
OPEN(UNIT=40,FILE=oifile,STATUS='unknown')
OPEN(UNIT=41,FILE=offile,STATUS='unknown')
DO ii=1,nra
   READ(50,*)ns
   READ(60,*)nr
   ...
   DO i=1,ns
      DO j=1,nr
         ...
         IF(ots(isum).EQ.1)THEN
            itres=otim(isum)
            IF(rmtr.EQ.1)THEN
               ftres=otim(isum)-(rdum-rtim(isum))+srcm(i)
            ELSE
               ftres=otim(isum)-(rdum-rtim(isum))
            ENDIF
            WRITE(40,*)itres
            WRITE(41,*)ftres
            IF(ABS(itres).GT.prs)THEN
               WRITE(6,1)recn(j),ii,i,itres
1              FORMAT('Recorder ',a7,' in array ',i3, &
              &' for source ',i5,' has a &
              &residual size of ',f7.4,' s.')
            ENDIF
            IF(ABS(ftres).GT.prs)THEN
               WRITE(6,2)recn(j),ii,i,ftres
2              FORMAT('Recorder ',a7,' in array ',i3, &
              &' for source ',i5,' has a &
              &residual size of ',f7.4,' s.')
            ENDIF
