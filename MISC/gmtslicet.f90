...
REAL*8, DIMENSION(:,:), ALLOCATABLE :: vela
REAL*8, DIMENSION(:,:,:), ALLOCATABLE :: veln
...
REAL :: rdep,rlat,rlon
REAL*8 :: gor,got,gop,rgsr,rgst,rgsp,sldep,slns,slew
REAL*8 :: tt,ttt,ttb,sumi,sumj,sumk
REAL*8 :: lft,rgt,btm,top
REAL*8 :: rdm,rd1,rd2,rd3,u,v,w
REAL*8, DIMENSION(:) :: wi(4)
REAL*8, DIMENSION(:,:), ALLOCATABLE :: ui,vi
REAL*8, DIMENSION(:,:,:), ALLOCATABLE :: ttn
REAL*8, PARAMETER :: dtol=1.0e-5
REAL*8, PARAMETER :: stol=1.0e-8
REAL*8 :: deltas,deltaskm,azim
REAL*8 :: deltanp,thgcp,phgcp,goti,gopi
REAL*8 :: slgclat1,slgclon1,slgclat2,slgclon2
REAL*8 :: slgclat1d,slgclon1d,slgclat2d,slgclon2d
...
DO i=0,nnp+1
   DO j=0,nnt+1
      DO k=0,nnr+1
         READ(20,*)veln(k,j,i)
!         write(*,*)veln(k,j,i),i,j,k,nnp,nnt,nnr
      ENDDO
   ENDDO
ENDDO
CLOSE(20)
...
!
   ...
   btm=gor-(nnr-1)*rgsr
   top=gor
   OPEN(UNIT=80,FILE=ofilegcb,STATUS='unknown')
   WRITE(80,'(f16.11)')lft
   WRITE(80,'(f16.11)')rgt
   WRITE(80,'(f16.11)')btm
   WRITE(80,'(f16.11)')top
   WRITE(80,'(I16)')ngch
   WRITE(80,'(I16)')ngcd
   WRITE(80,'(f8.3)')slgclat1d
   WRITE(80,'(f8.3)')slgclon1d
   WRITE(80,'(f8.3)')slgclat2d
   WRITE(80,'(f8.3)')slgclon2d
   CLOSE(80)
ENDIF
!
...
IF(pvgd.EQ.1.OR.pvgns.EQ.1.OR.pvgew.EQ.1.OR.pvggc.EQ.1)THEN
   ...
   IF(ABS(idm1-nnr).GT.dtol.OR.ABS(idm2-nnt).GT.dtol.OR.ABS(idm3-nnp).GT.dtol)isw=1
   READ(20,*)rd1,rd2,rd3
   !IF(rd1.NE.gor.OR.rd2.NE.got.OR.rd3.NE.gop)isw=1
   IF(ABS(rd1-gor).GT.dtol.OR.ABS(rd2-got).GT.dtol.OR.ABS(rd3-gop).GT.dtol)isw=1
   READ(20,*)rd1,rd2,rd3
   !IF(rd1.NE.rgsr.OR.rd2.NE.rgst.OR.rd3.NE.rgsp)isw=1
   IF(ABS(rd1-rgsr).GT.dtol.OR.ABS(rd2-rgst).GT.dtol.OR.ABS(rd3-rgsp).GT.dtol)isw=1
   IF(isw.EQ.1)THEN
...
!           This gives the velocity anomaly.
            !if(iout.eq.1)then
              !veln(k,j,i)=veln(k,j,i)
            if(iout.eq.0)then
              veln(k,j,i)=(veln(k,j,i)-rd1)/rd1*100
            endif
         ENDDO
      ENDDO
   ENDDO
