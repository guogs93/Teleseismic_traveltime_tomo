...
SUBROUTINE subadjoint(ishot,T,residuals,uu,adjn)
 ...
 ALLOCATE(ttn(0:nnp+1,0:nnt+1,0:nnr+1),nsts(nnp,nnt,nnr),lock(nnp,nnt,nnr))
 ...
 DEALLOCATE (ttn,nsts,adj,adjo,lock)
    ...
    IF(tstat(ishot,l).EQ.1)THEN
      ...
      IF(abs(par1).GT.(1.0d-10).AND.abs(par2).GT.(1.0d-12))THEN
        IF(adjn.EQ.1)THEN
          ...
          lock(irp-1,irt,irr)=0
          lock(irp+1,irt,irr)=0
          lock(irp,irt-1,irr)=0
          lock(irp,irt+1,irr)=0
          lock(irp,irt,irr+1)=0
        ELSE
...
SUBROUTINE adjoint_sweep
 ...
 DO swp=1, 4
    ...
    DO k=kmin(swp),kmax(swp),kstep(swp)  
       IF(nsts(k,j,i).EQ.0.and.lock(k,j,i).EQ.0)THEN
         ri=gor-(i-1)*dnr+earth
         risti=ri*sin(cgot+(j-1)*dnt)    
         ...
         ELSE
           ...
           IF(abs(par4).GT.1.0d-12)THEN
             ...
             lock(k,j,i)=1
             if(k+1.le.nnp)then
               if(ttn(k+1,j,i).le.ttn(k,j,i))lock(k+1,j,i)=0
             endif
             if(k-1.ge.1)then
               if(ttn(k-1,j,i).le.ttn(k,j,i))lock(k-1,j,i)=0
             endif
             if(j+1.le.nnt)then
               if(ttn(k,j+1,i).le.ttn(k,j,i))lock(k,j+1,i)=0
            endif
            if(j-1.ge.1)then
              if(ttn(k,j-1,i).le.ttn(k,j,i))lock(k,j-1,i)=0
            endif
            if(i+1.le.nnr)then
              if(ttn(k,j,i+1).le.ttn(k,j,i))lock(k,j,i+1)=0
            endif
            if(i-1.ge.1)then
              if(ttn(k,j,i-1).le.ttn(k,j,i))lock(k,j,i-1)=0
            endif
