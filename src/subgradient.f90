...
SUBROUTINE subgradient(dobs,velr,vel,vel1D,grad,pgrad,precon,cost,cost1,cost2)
 ...
 REAL*8,ALLOCATABLE:: gradsmo(:,:,:),pgradsmo(:,:,:),hesstv(:,:,:)
 ...
 ALLOCATE(gradt(nnp,nnt,nnr),hesst(nnp,nnt,nnr),pgradt(nnp,nnt,nnr))
 ALLOCATE(dcal(nsrc,nrc),residuals(nsrc,nrc))
 ALLOCATE(veln(nnp,nnt,nnr))
 ALLOCATE(gradtv(nvp,nvt,nvr),pgradtv(nvp,nvt,nvr),preconv(nvp,nvt,nvr),hesstv(nvp,nvt,nvr))
 ...
 DO ishot=1,  nsrc
    ...
    gradt(:,:,:)=gradt(:,:,:)+adj(:,:,:)/veln(:,:,:)**3
    hesst(:,:,:)=hesst(:,:,:)+padj(:,:,:)/veln(:,:,:)**3
 ...
 DO j=1, nnt
 DO k=1, nnr
    hesst(i,j,k)=log(1000*hesst(i,j,k)+1)
    if(crt.eq.1)then
      if(k.le.ncrt)then
        hesst(i,j,k)=0
      endif
 ...
 DO j=1, nvt
 DO k=1, nvr
    hesstv(i,j,k)=hesst(nrfp*(i-1)+1,nrft*(j-1)+1,nrfr*(k-1)+1)   
 ENDDO
 ENDDO
 ENDDO
 
 OPEN(UNIT=10,FILE='Hess.dat',STATUS='unknown')
 ...
 DO k=0, nvr+1
    ...
    WRITE(10,'(2f12.8)')hesstv(i,j,k),0.
    endif
 ENDDO
 WRITE(10,'(1X)')
 ENDDO
 ...
 write(*,*) 'write out preconditioner'
 ...
 DO j=1, nnt
 DO k=1, nnr
    WRITE(888,'(5F12.4)')gopd+(i-1)*dnpd,gotd-(j-1)*dntd,gor-(k-1)*dnr,gradsmo(i,j,k)/gradtmax
 ENDDO
 ENDDO
 ENDDO
 close(888)
 ...
 DO j=1, nnt
 DO k=1, nnr
    WRITE(888,'(5F12.4)')gopd+(i-1)*dnpd,gotd-(j-1)*dntd,gor-(k-1)*dnr,pgradt(i,j,k)/gradtmax
 ENDDO
 ENDDO
 ENDDO
 close(888)
 ...
 DEALLOCATE(adj,T,padj,gradt,hesst,pgradt)
 DEALLOCATE(dcal,residuals)
 DEALLOCATE(veln)
 DEALLOCATE(gradtv,pgradtv,hesstv)
